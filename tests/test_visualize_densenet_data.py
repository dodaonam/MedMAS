...
def _prediction_frame(run_id: str) -> pd.DataFrame:
    ...
    thresholds = thresholds_by_label(select_validation_thresholds(y_true, y_prob, TARGET_LABELS, run_id=run_id), TARGET_LABELS)
    rows = []
...
class DenseNetVisualizationDataTests(unittest.TestCase):
    def test_load_visualization_artifacts_validates_run_id(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
                ...
                [
                    {
                        ...
                        "val_all_label_macro_auroc": 0.5,
                        "val_all_label_macro_average_precision": 0.4,
                    }
                ]
            )
            ...
            y_true = frame[[f"true_{label_slug(label)}" for label in TARGET_LABELS]].to_numpy(dtype=int)
            y_prob = frame[[f"prob_{label_slug(label)}" for label in TARGET_LABELS]].to_numpy(dtype=float)
            thresholds = select_validation_thresholds(y_true, y_prob, TARGET_LABELS, run_id=run_id)
            save_json(paths.thresholds_path, thresholds)
            metrics = compute_multilabel_metrics(y_true, y_prob, thresholds_by_label(thresholds, TARGET_LABELS), TARGET_LABELS, run_id=run_id)
